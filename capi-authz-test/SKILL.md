...
- 修正が一見自明（読み取り系と同じ1行を書き込み系にも足すだけ等）に見えても、その場修正はレビュー（ステップ7相当の確認）や品質契約の再検証を経ないため、関連する見落とし（例：担当授業チェックだけ直して登録期間チェックの欠落を見逃す）を隠してしまう
