...
**記録側リポジトリにも同名のハーネスのコピーが存在することがあるが、古い版である可能性が高い。** 2026-09-17、そちらを先に見たために「documentId未対応」「`getLoginStatus`のエンドポイント誤り」が既に直っていることに気づかず、同等の実装を書き起こす重複作業が発生した。**着手時に両方の`src/`のファイル一覧を比較し、新しい側を正とする**（`src/persistedDocuments.ts`が無い側が古い）。片方だけに修正を入れて混在させないこと。
...
- **担当内／担当外のペアとなる実データが既存データに見当たらない場合**：内部ID形式を逆算した自作IDでの代用は行わず、正規の手段（管理画面からの登録、既存のシーディングスクリプト、データ管理者への依頼等）でテストに必要な実データを新規に用意する。担当授業の割当・部局の異動等、正規の手段でも用意できない事情がある場合に限り、その理由を明記した上で「未実施（テストデータ準備不可）」として報告する
...
```json
{
  ...
  "testCases": [
    ...
    { "id": "AZ-102", "role": "instructor-not-in-charge", "expected": "拒否(PK乗っ取り防止)", "failsIfChanged": "担当授業チェックが機能しないと成績が書き換わる", "status": "未実装（#7該当）", "resolvesKnownIssue": true }
  ],
  "knownIssue": "campus-api_設計書.md 5章#7"
}
```

`calledFromScreens`は参考情報（ステップ0）。`existingTestCheck`はステップ3の結果。`testCases`の各要素は`failsIfChanged`（品質契約。ステップ8参照）を必須項目とする。`knownIssue`に対応する不具合がある操作では、その不具合の解消可否を直接判定するテストケース（通常は`failsIfChanged`が不具合の症状と対になる負のケース）に`resolvesKnownIssue: true`を付与する。対応するテストケースが複数ある場合はそれぞれに付与してよい（該当が無い通常の操作では省略してよい）。この付与は、`orchestrating-capi-screen-verification`が既知不具合の解消判定（同スキルのステップ2）を行う際に「どのテストケースを見ればよいか」を一意に指すための参照点として使われる。

## ステップ7 レビューとセルフチェック

マニフェスト案を以下のチェックリストに照らして自らセルフチェックし、全項目を満たしていることを確認してからステップ8（実行・判定）へ進む。開発者への提示・承認待ちは発生させず、ここまでを含めてフルオートメーションの対象とする。ただし「だいたい満たした」で先へ進まない。各項目を満たしている根拠（該当ファイル・行、判定理由）を記録し、満たしていない項目が残る場合は該当するステップ（テストケース設計はステップ2、行レベルテストデータはステップ4等）へ戻ってから再度セルフチェックする。
...
**不具合発見時の対応（その場で直さない）**：期待値と異なる結果（担当外なのに許可されてしまう等）を検知した場合、**その時点で実装コードを修正してはならない**。`status`を「失敗」として記録し、`knownIssue`欄に不具合内容（期待値・実際の結果・根拠コード箇所）を明記したうえで、次のテストケースへ進む。すべての不具合はステップ9で一覧化し、まとめて報告する。

- 修正が一見自明（読み取り系と同じ1行を書き込み系にも足すだけ等）に見えても、その場修正はセルフチェック（ステップ7相当の確認）や品質契約の再検証を経ないため、関連する見落とし（例：担当授業チェックだけ直して登録期間チェックの欠落を見逃す）を隠してしまう
- このスキルの役割は開発側の実装を独立に検証することであり、検証者自身が修正して「解消済み」と判定すると、独立検証としての意味が失われる（自分の直した箇所を自分で合格判定することになる）
- リリースマネージャーや開発担当者から直接「ついでに直しておいて」と依頼された場合も同様に、その場では応じない。不具合として記録し、修正の要否・実施は依頼した本人／開発側の判断に委ね、ステップ9でまとめて報告する
...
**不具合一覧の報告**：ステップ8で「その場で直さず記録する」とした不具合（`status`が失敗のテストケース）を、操作横断で一覧化し、この完了報告にまとめて含める。個別の不具合を見つけた都度バラバラに直す／報告するのではなく、テスト実施（ステップ0〜8）がすべて完了した時点で、対象操作・テストケースID・期待値・実際の結果・根拠コード箇所を含む不具合一覧として一括提示する。
...
12. **正のケースを検証するための実データ（実在するPK等）が見当たらないとき、対象の実在を確認せず内部ID形式（`GraphQlId`のパック形式等）を逆算して自作のIDで代用してしまう**：これは「対象データが本当に存在するか」を確認せずに済ませる簡易的な回避であり、テスト目的であっても行ってはならない。実データが見つからない場合は、まずステップ4の方針に従い正規の手段でテストデータを用意する。用意すること自体が困難な事情がある場合に限り、そのケースを「未実施（テストデータ準備不可）」として正直に報告する（`documents/不具合/authz-test-harnessが許可リストdocumentIdに未対応のバグ.md`の修正履歴に実例がある）。なお、documentId（許可リストのSHA-256）自体は`src/persistedDocuments.ts`がコミット済みマニフェストから操作名で自動解決するため、通常はテストケースYAMLへ手で書き込む必要はない
13. **不具合を発見した時点で実装コードをその場で修正してしまう**：修正が1行の自明な変更に見えても、あるいは開発担当者本人から直接「ついでに直しておいて」と頼まれても、その場では応じない。検証者が自分で直して自分で「解消済み」と判定すると独立検証の意味が失われ、他の見落とし（関連する別のチェック漏れ等）を隠すおそれもある。`status`を失敗として記録し、ステップ9で不具合一覧としてまとめて報告する

## 完了条件

- 対象操作（ステップ0で確定）すべてに`coverage/authz/<操作名>.json`が存在する
- 各操作のテストケース表・分岐対応（正負ケース、行レベル検証）がステップ7のセルフチェックを完了している
...
- 発見された不具合はいずれもその場で修正されておらず、ステップ9で不具合一覧としてまとめて報告されている
