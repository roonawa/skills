...
新旧比較（ステップ9）は2つのチェックアウトを使い分ける。**探す前にここを見る**（ホームディレクトリだけを探して「新環境のリポジトリが無い」と誤断定した事例がある）。

| 役割 | リポジトリ | 実パス | 既定の`baseURL` |
|---|---|---|---|
| 旧版（移行前）＝記録対象 | `allcampus` | `C:\Users\roonawa\allcampus` | `https://allcweb3.local.ailesys.co.jp/campus/`（ST） |
| 新版（移行後）＝比較対象 | `allcampus-regression` | `D:\allcampus-regression\allcampus-regression` | `https://allcweb2.local.ailesys.co.jp/campus/`（PT） |
