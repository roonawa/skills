...
- **新環境（移行後の比較対象）＝PT環境**
- **リポジトリ対応**：ST環境＝`allcampus`リポジトリ（**`test-regression-branch`ブランチをチェックアウトしていること**）、PT環境＝`allcampus-regression`リポジトリ。新旧比較を伴う実行（ステップ9参照）ではこの対応関係に従い`allcampus`→`allcampus-regression`の順に実行する。実パスの解決手順は「リポジトリと環境の対応」の初回セットアップを参照
...
## 実データ・テストアカウントが揃わない場合の扱い（自作IDでの代替）

対象操作に必要なロール・担当範囲・データ件数の組み合わせ（例：特定部局に複数件の申請データを持つ経理担当）が、上表の既存テストアカウントとST環境の実データを確認しても用意できないことがある。**この場合も、対象操作のテスト実装を「保留」「要確認」「対象外」のまま終わらせてはならない。** 開発者・環境担当への確認・依頼と並行して、自らIDを作成する（アカウントの新規発行相当の代役・不足データに相当する自作データの用意）ことで代替し、その場でテストケース・マニフェスト・テストコードまで完成させる。

- まず既存アカウント表・ST環境の実データで条件を満たせるかを確認する順序自体は変えない（確認を飛ばしていきなり自作に進まない）
- 確認の結果、既存アカウント・既存データでは条件を満たせないと判明した場合に限り、自作アカウントID・自作データを作成して使う。これはCommon Mistakes #6が禁じる「その場しのぎ」とは別物である。同項が禁じているのは代替の理由・内容を記録に残さず場当たり的に済ませることであり、理由・内容を明記した上で計画的に自作するのは正当な代替手段である
- 検索・帳票出力における0件許容除外（ステップ3）も同様に扱う：既存データで複数件の状態を確認できない場合、0件のまま記録して済ませるのではなく、自作データで複数件の状態を作ってから記録する
- マニフェスト（ステップ6）の該当エントリに、自作IDで代替した旨と理由を明記する（例：`"testDataSource": "self-created"`を追加し、`evidence`に作成したアカウント・データの概要を書く）。既存アカウント・実データで検証したケースと区別できるようにする
- 開発者・環境担当への正規のアカウント発行・データ投入依頼は妨げない。並行して行ってよいが、その回答を待つこと自体をテスト実装を止める理由にしない（回答を待つ間に自作IDで先に完成させ、正規の対応が届いた時点で差し替えが必要かを判断する）

### よくある言い訳と実際

| 言い訳 | 実際 |
|---|---|
| 「該当データが0件しか無いので記録できない」 | 自作データで複数件の状態を作れば記録できる。0件のまま記録するか諦めるかの二択ではない |
| 「自作アカウントはCommon Mistakes #6が禁止している」 | #6が禁止するのは理由を残さない場当たり的な作成。既存確認済みの上で理由・内容を明記して作るのは正当な代替であり、`testDataSource`に明記すればよい |
| 「期限までに間に合わないので保留にする」 | 保留にすること自体がその画面のマニフェスト・テストケース表を未完成にする。自作IDで完成させた上で報告するほうが、根拠不明の保留より進捗として正しい |

**Red Flags（これが出たら自作IDでの代替に進む。保留・対象外の理由にしない）：**
「該当するロール・データが実データ中に見当たらない」／「既存アカウント表にこの組み合わせが無い」／「開発者の回答待ちのため保留」／「0件しか出ないので対象外」

## リポジトリと環境の対応

新旧比較（ステップ9）は2つのチェックアウトを使い分ける。**実パスは配布先（実行者）ごとのディレクトリ構造に依存するため、本ファイルに固定値として書かない。** 実パスは初回セットアップ（次項）で実行者ごとに解決し、解決結果を下表の「実パス」列に**本ファイル自身を書き換える形で**記載する。**探す前にここを見る**（ホームディレクトリだけを探して「新環境のリポジトリが無い」と誤断定した事例がある）。

| 役割 | リポジトリ | 必須ブランチ | 既定の`baseURL` | 実パス |
|---|---|---|---|---|
| 旧版（移行前）＝記録対象 | `allcampus` | **`test-regression-branch`**（既定ブランチのままでは新旧比較の前提〈記録側と同じ内容のST環境〉が成立しない） | `https://allcweb3.local.ailesys.co.jp/campus/`（ST） | `C:\Users\roonawa\allcampus` |
| 新版（移行後）＝比較対象 | `allcampus-regression` | 指定なし（実行者の作業ブランチをそのまま使う） | `https://allcweb2.local.ailesys.co.jp/campus/`（PT） | `D:\allcampus-regression\allcampus-regression` |

- Playwrightの作業ディレクトリはいずれも `frontend/campus`。依存は `frontend/node_modules` に巻き上がっているため、`frontend/campus/node_modules` が空でも実行できる。`npm install` が husky の prepare で失敗する場合は `--ignore-scripts` を付ける
- 新環境側リポジトリは記録側より**古いコピーであることがある**。新旧比較は「同じテストコードを両環境へ」実行するのが前提なので、spec・templates・マニフェストは記録側（`allcampus`）から新環境側へ同期してから実行する。`playwright.config.ts` と `templates/auth.ts`（アカウントの環境変数名が違う）は同期しない
- 新環境側リポジトリには `coverage/screens/` が無いことがある。捕捉条件の解決はマニフェスト参照に依存するため、**マニフェストを配置しないと spec が読み込み時に例外で落ちる**

### 初回セットアップ（実行者ごとのリポジトリパス解決・一度だけ）

上表の「実パス」列がプレースホルダ（`<初回セットアップで解決・記載>`）のまま、または記載済みのパスに`.git`が存在しない・リポジトリ名が一致しない場合、以下を実施してから本スキルの他のステップへ進む。**一度解決すれば以後の実行では再探索しない**（上表に書き込まれた実パスをそのまま使う）。

1. ホームディレクトリ直下、次に主要なドライブのルート付近を探索し、フォルダ名が`allcampus`（`-regression`が付かないもの）と`allcampus-regression`で、それぞれ`.git`を持つディレクトリを探す
2. 複数候補が見つかった場合、または1件も見つからない場合は、推測で決め打ちせず実行者に実パスを直接尋ねる
3. `allcampus`（旧版・ST環境）側で`git status`を確認する。未コミットの変更が無いことを確認したうえで、現在のブランチが`test-regression-branch`でなければチェックアウトする（ローカルに存在すればそのまま、無ければ`git fetch`後に`git checkout test-regression-branch`、リモートにしか無ければ`git checkout -b test-regression-branch origin/test-regression-branch`）。未コミットの変更がある場合はチェックアウトを強行せず、状況を実行者に報告して指示を仰ぐ（無断でstash・破棄しない）
4. `allcampus-regression`（新版・PT環境）側はブランチ指定が無いため、チェックアウト操作は行わない（現在のブランチをそのまま使う）
5. 解決した実パスを、本ファイル（`campus-playwright/SKILL.md`）の上表「実パス」列に書き込む（Editツールで該当セルを更新する）

**配布・共有時の注意**：この「実パス」列は実行者ごとに異なるローカル環境固有の値であり、配布元（`.claude/skills`が git 管理下にある場合はそのリポジトリ）へコミット・プッシュしない。書き込み後、`git update-index --skip-worktree campus-playwright/SKILL.md` 等でこのファイルへのローカル変更を追跡対象から外しておくと、以後の`git status`やpullで実パスが誤って共有・上書きされるのを防げる（本スキルの内容自体を編集する際は、先に`--no-skip-worktree`へ戻し、実パス列をプレースホルダへ戻してから変更・コミットする）。
...
**ただし検索処理・帳票出力を伴う操作には、この「0件許容」を適用しない。** これらの操作は「返ってきたデータの内容そのもの」（内部データ・帳票の記載内容）の整合性を検証することが目的であり、0件の結果ではその検証ができず特性テストとして機能しない。検索条件・対象データの選定時にST環境の実データを確認し、**必ずある程度の件数（複数行・複数ページ分等）が表示される状態**をシナリオとして選び、それをエビデンス（画面表示・内部データ・帳票ファイル）として記録する。該当条件を満たす実データがST環境に無い場合は、0件のまま記録して済ませたり対象外・保留にしたりせず、「実データ・テストアカウントが揃わない場合の扱い」に従い自作データで代替する。
...
セルフチェック済みマニフェスト・テストケース表に基づき、共通テンプレート（ロール別ログインヘルパー、ネットワーク応答キャプチャのユーティリティ、`capturing-playwright-evidence`の撮影規約（el-dialog全体撮影ヘルパー・ヘッダー非表示での撮影・`<screen>_<role>_<シナリオ>.png`命名規則）に準拠する撮影ヘルパー、あらゆる操作の後に0.5秒待機する操作ヘルパーを持つ）を用いて、テストケースごとにPlaywrightテストコードを生成する。
...
- **捕捉マッチ条件の外部化**：ネットワーク応答を捕捉する際のマッチ条件（GraphQLの`operationName`、帳票ダウンロードのURLパスの両方）は、テストコード中に文字列リテラルとして直書きしない。マニフェスト（ステップ6）の該当エントリ（`operations[]`・`reports[]`いずれも）を実行時に参照し、下記「実行対象による解決の切り替え」に従ってマッチ条件を決める。`capi-changes`で操作名やダウンロードURLがリネームされても、マニフェストの`newOperationName`を埋めるだけでテストコード自体を書き換えずに`capi-regression-test`側の再実行に追従できるようにするため

#### 実行対象による解決の切り替え（`newOperationName ?? name` だけでは壊れる）

`newOperationName`を無条件に優先すると、**埋めた瞬間に旧版（ST環境）に対する実行が壊れる**（新操作名はST環境では一度も発生せず、待ち受けがタイムアウトする。2026-09-17に実際に発生）。マニフェストの1フィールドでは「どちらの環境に向けた実行か」を表現できないため、解決は実行対象で切り替える。

| 実行対象 | 使うマッチ条件 |
|---|---|
| 旧版（ST環境） | `name`（記録時点の識別子） |
| 新版（PT環境） | `newOperationName ?? name` |

実行対象は環境変数（`E2E_TARGET=old|new`）で明示し、未指定時は`baseURL`のホスト名から推定する（`allcweb3`＝ST＝旧版／`allcweb2`＝PT＝新版）。解決処理は共通ヘルパー（`templates/manifestOperations.ts`）に置き、spec側では直接`newOperationName`を読まない。新版で1操作が複数へ分割された場合、どれを待つかはシナリオによって変わるため、ヘルパーは候補の絞り込み（部分文字列指定等）も受けられるようにする。

#### 新版は `operationName` を送らない（documentIdで逆引きする）

**これは`newOperationName`を正しく設定しても解決しない、機構そのものの制約である。** campus専用APIのトランスポート（`src/services/campus/CampusTransport.ts`）は送信本文を次の形に固定しており、**`operationName`を送らず、`query`も操作の中身を含まない固定プレースホルダ**である（実際に実行される本文はサーバが`documentId`から引く）。

```json
{ "query": "query campusPersistedQuery { __typename }", "variables": {}, "extensions": { "documentId": "<sha256>" } }
```

したがって**新版に向けた捕捉は、リクエスト本文の`operationName`でも`query`の解析でも識別できない。** `extensions.documentId`を、コミット済みの許可リストマニフェスト（`backend/campus/src/main/resources/graphql/persisted/{campus-api,campus-login}.json`）で操作名へ逆引きする経路を捕捉ヘルパー（`templates/graphqlCapture.ts`）に持たせる。旧版（既存API）は従来どおり本文に`operationName`を載せるため、両方に対応した判定にする（`operationName`があればそれを使い、無ければ`documentId`から解決する）。

この対応を欠くと、PT環境に対する実行は**すべて「応答が来ない」タイムアウトになり、差分ではなく偽陰性**になる（Common Mistakes #14）。
...
11. **あらゆる操作の後の0.5秒待機を省略する。** ダイアログの開閉時等、opacityの切り替わりの途中でUIが正しく表示されない状態のまま次の操作・撮影に進んでしまい、崩れたエビデンスや不安定な待機条件によるテスト失敗を招く。共通テンプレートの操作ヘルパーはクリック・入力・タブ切替等あらゆる操作の直後に`page.waitForTimeout(500)`等で0.5秒待機してから次に進む（`capturing-playwright-evidence`の撮影規約と共通）
...
6. **テストアカウントをその場で都度用意する**：既に実機検証済みの安定アカウント（「ST環境テストアカウント」参照）を優先して使い、再現性を確保する。既存アカウントで条件を満たせないと確認できた場合の自作IDによる代替は「実データ・テストアカウントが揃わない場合の扱い」を参照（理由・内容を記録した計画的な代替はここでいう「その場しのぎ」ではない）
...
15. **検索処理・帳票出力のシナリオで、結果0件のデータ状態をそのままエビデンスにしてしまう**：0件許容の例外（ステップ3）は画面の初期表示等に限った話であり、検索・帳票出力ではそもそも「返ってきたデータの内容」の整合性確認が目的のため0件では検証にならない。シナリオ選定時にST環境の実データを確認し、複数件が表示・出力される条件を選ぶ。該当する実データが無い場合も0件のまま済ませず、自作データで代替する（「実データ・テストアカウントが揃わない場合の扱い」参照）
...
19. **新版のリクエストを`operationName`で捕捉しようとする**：新版のトランスポートは`operationName`を送らず`query`も固定プレースホルダなので、**マニフェストの`newOperationName`を正しく埋めても捕捉できない**。`extensions.documentId`を許可リストマニフェストで逆引きする経路が捕捉ヘルパーに必要（ステップ8「新版は`operationName`を送らない」）。これを機構の不足と気づかず「新版で操作が発生していない」と誤読しない
20. **`newOperationName`を埋めたまま旧版（ST）に対して実行する**：新操作名はST環境では発生しないため、マニフェストを埋めた瞬間に旧版実行がタイムアウトで壊れる。解決は実行対象（`E2E_TARGET`）で切り替える（ステップ8「実行対象による解決の切り替え」）
21. **環境ごとに変わる値をテストコードや特定環境の`.env`固定で読む**：静的アセットのURL（`VITE_ASSET_PDF_URL`等）はST/PTでホストが変わるため、`.env.pt`を固定で読むと記録対象がSTへ変わった時点で「期待 allcweb2 / 実測 allcweb3」で必ず失敗する。実行対象に対応する環境ファイルから読む（ステップ8「環境の外部化」）
22. **新環境側リポジトリが記録側より古いことを確認せずに新旧比較を始める**：新環境側の spec が旧いままだと「同じテストコードを両環境へ」という前提が崩れる。spec・templates・マニフェストを同期してから実行する（「リポジトリと環境の対応」参照）
23. **`allcampus`（旧・ST環境）が`test-regression-branch`以外のブランチのまま新旧比較を実行してしまう**：既定ブランチのままでは記録側と同じ内容のST環境という前提が成立せず、比較結果の意味が保証できない。初回セットアップ（「リポジトリと環境の対応」）でブランチを確認・チェックアウトし、以後の実行前にも現在のブランチが`test-regression-branch`のままであることを都度確認する
24. **実パスをSKILL.md本体にハードコードしたまま配布し、他の実行者の環境で無条件に使ってしまう／配布元リポジトリへ実パスの差分をコミットしてしまう**：実パスは実行者ごとのディレクトリ構造に依存する。初回セットアップの手順（探索→確認→本ファイルへの書き込み）を省略しない。書き込んだ実パスは各実行者のローカル環境固有の情報であり、配布元へコミット・プッシュしない
25. **実データ・テストアカウントが揃わないことを理由に、テストを保留・対象外・要確認のまま完成させない**：開発者・環境担当への確認依頼自体は正当だが、その回答待ちをテスト未実装の言い訳にしない。既存アカウント・既存データで条件を満たせないと確認できた場合は自作アカウントID・自作データで代替し、その場でテストケース・マニフェスト・テストコードまで完成させる（「実データ・テストアカウントが揃わない場合の扱い」参照）
...
- 新旧比較を行う場合、初回セットアップ（「リポジトリと環境の対応」）で実パスが解決済みであり、`allcampus`が`test-regression-branch`をチェックアウトしていることを確認済みである
...
- 実データ・テストアカウントが揃わないことを理由に保留・未実装のまま残っている操作が無い（自作IDでの代替を含め、すべてマニフェストの`operations`/`reports`/`acceptedRisks`/`excluded`のいずれかに確定している）
