...
|---|---|---|---|---|
| 旧版（移行前）＝記録対象 | `allcampus` | **`test-regression-branch`**（既定ブランチのままでは新旧比較の前提〈記録側と同じ内容のST環境〉が成立しない） | `https://allcweb3.local.ailesys.co.jp/campus/`（ST） | `<初回セットアップで解決・記載>` |
| 新版（移行後）＝比較対象 | `allcampus-regression` | 指定なし（実行者の作業ブランチをそのまま使う） | `https://allcweb2.local.ailesys.co.jp/campus/`（PT） | `<初回セットアップで解決・記載>` |
