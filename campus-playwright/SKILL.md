...
- **カバレッジ数値による完了判定は行わない。** 対象はマニフェスト（ステップ6）とテストケース表（ステップ3）で管理し、承認済みマニフェストの`operations`/`reports`全項目に対応する正常系テストが揃い、ST環境に対して成功していることをもって完了とする（ステップ9・完了条件）
...
**After**：ソースコードから機械的に抽出し、画面単位のマニフェストで対象・除外を確定→レビュー承認→生成→**ST環境に対する実行結果で完了判定**、という一直線のプロセスにする。
...
**認証方式も変わる。** `backend/api/src/main/resources/config/application-st.yml`は`use-local: true`（`application-pt.yml`は`use-local: false`）であるため、ST環境では`/login`画面がSAMLリダイレクトではなくID/パスワードのログインフォーム（`.login-id`/`.login-pass`の`el-input`、`Login.vue`）を表示する（**本改訂時に実機確認済み**）。これにより学生・大学院生・教員・職員も`loginByLocal`（画面のログインフォーム操作）で認証できる見込みが立った。

**ただし「PT環境テストアカウント」表のID・パスワードをそのままST環境へ流用することはできない（本改訂時に実機確認済み・失敗）。** 学生アカウント`ntatsuno+11@ailesys.co.jp`（PT用パスワード）でST環境の`/login`を試したところ「ログインIDまたはパスワードが間違っているため、ログインができませんでした。」で失敗した。ST環境はPT環境と別データベースのため、ログイン可能なID・パスワードの組自体をST環境向けに新たに用意する必要がある。新規に画面へ着手する際は、まず対象ロールのST環境向けアカウントの有無を確認し（無ければ開発者に確認・払い出しを依頼する）、実機で`loginByLocal`が通ることを確認してから「ST環境テストアカウント」表を更新すること（推測で決め打ちしない。本スキル基本原則と同じ姿勢）。SAMLでしか認証できないロール・環境が残る場合のみ`capi-saml-login`を使う。

## ST環境テストアカウント

**記録（テスト実行）は開発環境ではなくST環境を使用する。** 下表はPT環境（向き先変更前）で実機検証済みだった安定アカウントの一覧であり、参考として残しているが、**IDはいずれもST環境向けに未払い出しの可能性が高い（学生アカウントは本改訂時に実機確認済みで実際に失敗した）。** ST環境はPT環境と別データベースであり、ログイン方式が有効になっていること（`use-local: true`）と、そのロールの実在するアカウントが払い出されていることは別問題である。**新規に画面へ着手する際は、対象ロールのST環境向けアカウント（ID・パスワード）を開発者に確認・払い出し依頼したうえで、実機で`loginByLocal`等が通ることを確認してから本表を更新すること。** パスワードは本ファイルに記載しない（`C:\Users\roonawa\pw\.env`に保存済み。テストコード・マニフェストへの直書きも禁止し、環境変数経由で読み込む）。

| 身分 | ID（PT時点。ST向けに要再払い出し） | ログイン方法 | 想定ロール |
|---|---|---|---|
| 学生 | `ntatsuno+11@ailesys.co.jp` | PTではSAML経由（`capi-saml-login`）必須だった。STは`use-local: true`のためログイン画面自体はID/パスワード方式になるが、**このIDのパスワードでは本改訂時に実機確認した結果ログイン失敗**（別データベースのため未払い出しと推測） | `ROLE_STUDENT` |
| 大学院生 | `ntatsuno+10@ailesys.co.jp` | 同上（未確認。学生同様に失敗する可能性が高い） | `ROLE_STUDENT`（学部生と同ロール。区分コード等で判定が分かれる操作は別途確認） |
| 教員 | `info+gakusei03@ailesys.co.jp` | 同上（未確認） | `ROLE_INSTRUCTOR` |
| 職員 | `ntatsuno+3@ailesys.co.jp` | 同上（未確認） | `ROLE_STAFF` |
| 求人情報（卒業生WEB検索・OB） | `18J1090F` | 直接ログイン（`loginForJobob`。SAML不要）だが、ST環境での有効性は未確認 | `ROLE_JOBOB` |
| 一般市民 | `shimin2` | 直接ログイン（`loginForCitizen`。SAML不要）だが、ST環境での有効性は未確認 | `ROLE_CITIZEN` |
| 企業担当 | `K032496000` | 直接ログイン（`loginForKigyou`。SAML不要）だが、ST環境での有効性は未確認 | `ROLE_KIGYOU` |

**REQUIRED SUB-SKILL: capi-saml-login** — ST環境で`loginByLocal`が使えないロール・操作が見つかった場合のみ使用する（詳細はステップ8）。求人情報・一般市民・企業担当の3ロールは元々SAML不要で、画面のログインフォーム操作でそのままログインする方式自体は環境を問わず変わらない見込みだが、上記のとおりST向けのアカウント実在は別途確認が必要。
...
承認済みマニフェスト・テストケース表に基づき、共通テンプレート（ロール別ログインヘルパー、ネットワーク応答キャプチャのユーティリティ、`capturing-playwright-evidence`の撮影規約（el-dialog全体撮影ヘルパー・ヘッダー非表示での撮影・`<screen>_<role>_<シナリオ>.png`命名規則）に準拠する撮影ヘルパー、あらゆる操作の後に0.2秒待機する操作ヘルパーを持つ）を用いて、テストケースごとにPlaywrightテストコードを生成する。

- **ロール別ログインヘルパーの使い分け**：ST環境では`use-local: true`のため、原則として全ロールとも画面のログインフォーム操作（`loginByLocal`／`loginForJobob`／`loginForCitizen`／`loginForKigyou`）でそのままログインする（「ST環境テストアカウント」参照）。実機確認の結果、特定ロール・特定環境（PT環境での新版比較時等）でSAMLログインしか通らないことが判明した場合のみ、`capi-saml-login`でCookie・CSRFトークンを取得しブラウザコンテキストへ設定する方式に切り替える（毎回SAMLのUI操作をやり直さない）。いずれの認証情報も`.env`等から環境変数経由で読み込み、テストコードに直書きしない
...
テストコード生成後、実際にST環境に対して実行し、結果を報告する。「作成した」という自己申告では終わらせない。
...
- マニフェストの`operations`・`reports`・`acceptedRisks`全項目に対応する正常系テストコードが生成され、ST環境に対して実行し成功している
